...
void delay_init()
{
	...
	SysTick_CLKSourceConfig(SysTick_CLKSource_HCLK_Div8);	//选择外部时钟  HCLK/8
	fac_us=SystemCoreClock/8000000;				//为系统时钟的1/8  ,8000000

//	reload=SystemCoreClock/8;				//每秒钟的计数次数 单位为K	   
	reload=fac_us*1000;		//根据delay_ostickspersec设定溢出时间，每秒钟中断1000次
												//reload为24位寄存器,最大值:16777216,在72M下,约合1.86s左右	
	fac_ms=fac_us*1000;			//代表OS可以延时的最少单位	   
